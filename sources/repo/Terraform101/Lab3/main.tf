...
resource "azurerm_storage_account" "main" {
  name                     = "st${var.application_name}${var.environment_name}${local.region_code}002"
...
resource "azurerm_storage_container" "paulomonteiro" {
  name                  = "blobpaulomonteiro"
...
resource "azurerm_log_analytics_workspace" "main" {
  name                = "log-analytics-${var.application_name}-${var.environment_name}-${local.region_code}-001"
  location            = azurerm_resource_group.main.location
  tags                = local.default_tags
  resource_group_name = azurerm_resource_group.main.name
  sku                 = "PerGB2018"
